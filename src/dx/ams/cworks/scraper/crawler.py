...
class StrWrapIterator:
    "Wrap an int/float iterator (or even a mix of int and str) as str, with no generator"
...
def GET_book_metadata_pages(initialise_at=1, volumes=None, sort=True, dry_run=False):
    ...
    if sort:
        series_dict = dict(sorted(series_dict.items(),
            key=lambda item: [int(i) for i in item[0].split(".")])
        )
    if volumes is None:
        volumes = [k.replace(".", "-") for k in series_dict.keys()]
    ...
    for k in volume_iterator:
        k_sub = k.replace("-", ".")
        if float(k_sub) < initialise_at:
            continue
        suffix_list = [series_suffix, k]
        # title-case `true`/`false` then `eval` as bool
        is_hardback = eval(series_dict.get(k_sub).title())
        if is_hardback:
            suffix_list.append(hardback_suffix)
